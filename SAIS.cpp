...
template <typename T>
void printVector(const string &title, vector<T> &arr)
{
    if (arr.empty())
    {
        cout << title << ": []" << endl;
...
{
    size_t n = text.size();
    vector<char> res(n, '$'); // result vector that will store the types of the suffixes
    res.back() = 'S';         // last char is always S type for null character

    if (text.empty())
    { // return an array with one S-type for null character if string is empty
        return res;
    }

    res[res.size() - 2] = 'L'; // if string is not empty, 2nd-to-last char is always L-type

    // step through rest of string from right to left and set type based on next char
    for (int i = n - 2; i >= 0; i--)
    {
        if (text[i] > text[i + 1])
        { // if current char is greater than next, it's an L-type
            res[i] = 'L';
        }
        else if (text[i] == text[i + 1] && res[i + 1] == 'L')
        { // if current char is the same as the next char, current char is L-type only if the next char is an L-type
            res[i] = 'L';
        }
        else
        {
...
/*
Returns a map, where the key is the index, and the value is which bucket it belongs to.
*/
vector<size_t> findLMSSuffixes(const vector<size_t> &text, vector<char> &typemap)
{
    vector<size_t> res;
...
{
    vector<size_t> res(uniqueChars); // stores occurrences of each char (ascii has 128 characters)
    for (size_t i = 0; i < text.size(); i++)
    {
        res[text[i]]++;
    }
...
vector<size_t> findBucketHeads(const vector<size_t> &bucketSizes, size_t uniqueChars)
{
    size_t offset = 0;
...
vector<size_t> findBucketTails(const vector<size_t> &bucketSizes, size_t uniqueChars)
{
    size_t offset = 0;
    vector<size_t> res(uniqueChars);
    for (size_t i = 0; i < bucketSizes.size(); i++)
...
vector<size_t> guessLMS(const vector<size_t> &text, const vector<size_t> &bucketSizes, const vector<size_t> &LMSSuffixes, const vector<char> &typemap, size_t uniqueChars)
{
...
void induceSortL(const vector<size_t> &text, vector<size_t> &SA, const vector<size_t> &bucketSizes, const vector<char> &typemap, size_t uniqueChars)
{
...
void induceSortS(const vector<size_t> &text, vector<size_t> &SA, const vector<size_t> &bucketSizes, const vector<char> &typemap, size_t uniqueChars)
{
    vector<size_t> bucketTails = findBucketTails(bucketSizes, uniqueChars);
    // add S-types
...
bool lmsSubstringsAreEqual(const vector<size_t> &text, const vector<char> &typemap, size_t offsetA, size_t offsetB)
{
    size_t n = text.size();
...
summary reduceString(const vector<size_t> &text, const vector<size_t> &SA, const vector<char> &typemap)
{
    summary s;
...
vector<size_t> accurateLMSSort(const vector<size_t> &text, const vector<size_t> &bucketSizes, const vector<char> &typemap, const vector<size_t> &summarySA, const vector<size_t> &summarySuffixOffsets, size_t uniqueChars)
{
